...
Sono riuscito con non poche difficoltà a scrivere un programma per sostituire i testi di Katana ZERO con la traduzione italiana ufficiale presente nella versione da telefono.

**ATTENZIONE: Questa patch è stata testata solo sulla versione di gioco "v1.0.5", build "3889288", l'ultima disponibile**

Comunque, dovrebbe funzionare su qualsiasi versione del gioco, ma con la possibilità che alcune frasi non vengano tradotte.
...
Inoltre, per sistemi Linux, è in rari casi necessario installare la libreria `libxcb-cursor0` (usando il package manager della distribuzione installata) per poterlo eseguire.

Se il programma dovesse essere segnalato come malware, è consigliato disabilitare temporaneamente Windows Defender, in quanto si tratta di un falso positivo (molto comune nei file compilati di python). Il codice sorgente completo del programma è disponibile [qui](./patcher/patcher.py).
...
## Q&A
### Ho trovato errori nella traduzione, come posso segnalarli?
...
### Cosa ne sarà di questa traduzione una volta uscito il DLC?
...
- Patcha il file `data.win` con uno dei file `datawin_*.xdelta` presenti [qui](./patches), usando lo strumento [xdelta](https://github.com/jmacd/xdelta) (Qui avviene la modifica dei font usati, in modo tale da poter utilizzare i caratteri accentati "í" ed "ó").
